...
.service('cfLocation', ['$window', function ($window) {
  var encodeParam = function (prop, value) {
    var key = encodeURIComponent(prop);
    if (angular.isUndefined(value) || value === null || angular.isArray(value)) return null;
  ...
  this.encodeUrl = function (path, params) {
    ...
    if (angular.isUndefined(params)) return path;
    if (!angular.isObject(params)) throw new Error("params has to be an object.");
